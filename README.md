# Lumbung (LMB) — logo assets

Aset logo resmi untuk token **Lumbung (LMB)** di Robinhood Chain (chain ID 4663).
...
URL publik (raw, HTTPS, `Content-Type: image/png`):

```
https://raw.githubusercontent.com/DigitalKreasi25/lumbung-assets/master/mark_gold_green_512.png
```

Dipakai untuk field `logo` pada `TokenParams` saat launch via Pons V2.

Nama, simbol, dan logo ini adalah identitas proyek komunitas; tidak ada hubungan afiliasi dengan
Robinhood Markets, Inc. maupun penerbit USDG (Global Dollar / Paxos).
